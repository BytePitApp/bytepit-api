...
def list_users(current_admin_user: Annotated[UserDTO, Depends(get_current_admin_user)]):
    return get_users()


@router.get("/unverified-organisers", response_model=List[UserDTO])
def list_unverified_organisers(current_admin_user: Annotated[UserDTO, Depends(get_current_admin_user)]):
    return get_unverified_organisers()


@router.post("/confirm-organiser/{username}")
def confirm_organiser(username: str, current_admin_user: Annotated[UserDTO, Depends(get_current_admin_user)]):
...
def change_role(
    ...
    current_admin_user: Annotated[UserDTO, Depends(get_current_admin_user)],
):
